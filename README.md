...
**Bridge between Claude Code and Figma** — Create, read, modify, and delete design elements in Figma directly from Claude Code using natural language.
...
- **Read** node properties and selections
- **Modify** any element (position, size, colors, auto-layout, etc.)
- **Delete** nodes
- **Convert** frames to components
- **Replicate websites** with screenshot + HTML/CSS extraction
...
| `get_figma_node` | Get properties of a node by ID |
| `list_figma_nodes` | List nodes in page or children of a node |
